...
function drawHeader(page: PdfPage, invoice: PrintInvoice) {
  const brandInitial = cleanText(invoice.enterprise.name).slice(0, 1).toUpperCase() || "B"
  addBox(page, 36, 39, 36, 36, [224, 242, 254], [186, 230, 253])
  addText(page, 48, 64, brandInitial, 18, true, BLUE)
  addText(page, 84, 42, invoice.enterprise.businessType, 8, true, BLUE)
  addText(page, 84, 61, invoice.enterprise.name, 24, true)
  addText(page, 84, 78, invoice.enterprise.address, 9, false, MUTED)
  ...
  if (businessDetails) {
    wrapText(businessDetails, 52, 2).forEach((line, index) => {
      addText(page, 84, 93 + index * 12, line, 9, false, MUTED)
    })
  }
...
function drawTotals(page: PdfPage, invoice: PrintInvoice, y: number) {
  ...
  invoice.terms.slice(0, 3).forEach((term, index) => addText(page, 50, startY + 39 + index * 14, term, 9, false, MUTED))
...
function drawGeneratedByFooter(page: PdfPage, pageNumber: number, pageCount: number) {
  addRule(page, 36, 796, 559, 796, 0.7, BORDER)
  addText(page, 182, 814, "Thank you for your business.  |  Generated by Bezgrow", 8, true, MUTED)
  addText(page, 510, 814, `Page ${pageNumber} of ${pageCount}`, 8, false, MUTED)
}
...
export function createInvoicePdf(invoice: PrintInvoice) {
  ...
  pages.forEach((pdfPage, index) => drawGeneratedByFooter(pdfPage, index + 1, pages.length))
