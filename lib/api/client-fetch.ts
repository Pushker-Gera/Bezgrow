...
function withSelectedFinancialYear(input: RequestInfo | URL, init: RequestInit) {
  ...
  // Historical selection is a read filter only. Dated writes are assigned by
  // their accounting date in the local domain service, never by UI state.
  if (method !== "GET" && method !== "HEAD") return { input, init }
