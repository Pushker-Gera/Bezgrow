...
export function createProfessionalInvoiceMessage(invoice: PrintInvoice) {
  ...
  const lines = [
    ...
    "",
    "*Invoice Details*",
    `Customer: ${present(invoice.customer.name)}`,
    `Business: ${present(invoice.enterprise.name, "Business")}`,
    `Invoice Number: ${present(invoice.invoiceNumber, "Invoice")}`,
    `Invoice Date: ${formattedDate(invoice.invoiceDate)}`,
  ]
  lines.push("", "*Items*")
  ...
  invoice.items.forEach((item, index) => {
    ...
    lines.push(
      ...
      `Quantity: ${quantity(item.quantity)} ${present(item.unit, "unit")}`,
      `Rate: ${money(item.rate)}`,
    )
    if (present(item.hsnCode, "")) lines.push(`HSN: ${present(item.hsnCode)}`)
    if (gstPercent > 0) lines.push(`GST Rate: ${gstPercent.toLocaleString("en-IN", { maximumFractionDigits: 2 })}%`)
  ...
  lines.push("Thank you for your business.", "", "Generated by Bezgrow")
  return lines.join("\n")
}
...
export function createInvoiceEmailDraft(input: InvoiceShareInput) {
  ...
  const body = [
    ...
    input.secureInvoiceUrl
      ...
      : "This invoice was generated securely by Bezgrow.",
    "",
    "Thank you,",
