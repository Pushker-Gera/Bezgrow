...
export function createInvoiceShareText(input: InvoiceShareInput) {
  const greeting = `Hello ${input.customerName || "Customer"}, your invoice ${input.invoiceNumber || "Invoice"} from ${input.enterpriseName || "our business"} is ready. Total: ${money(input.amount)}.`
  const lines = [greeting, ""]
  if (input.secureInvoiceUrl) {
    lines.push(`View it securely here: ${input.secureInvoiceUrl}`)
  } else {
