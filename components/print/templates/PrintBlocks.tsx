  ...
  return (
    <header className="print-header-block">
      <div className="brand-block">
        {settings.showLogo && <BusinessLogo invoice={invoice} />}
        <div>
...
export function CodesBlock({ invoice, settings }: { invoice: PrintInvoice; settings: PrintSettings }) {
  return (
    <section className="codes-block" aria-label="Invoice reference codes">
      {settings.showBarcode && (
        <figure className="invoice-code invoice-barcode" data-code-kind="barcode">
          <Barcode value={invoice.barcodeValue} height={34} width={1.2} fontSize={9} margin={0} />
          <figcaption>{invoice.invoiceNumber}</figcaption>
        </figure>
      )}
      {settings.showQr && (
        <figure className="invoice-code invoice-qr" data-code-kind="qr">
          <QRCodeSVG value={invoice.qrValue} size={64} marginSize={1} />
...
export function GeneratedByFooter({ compact = false }: { compact?: boolean }) {
  return (
    <footer className={`generated-by-footer ${compact ? "compact" : ""}`}>
