...
export default function AppUpdatesPanel() {
  ...
  async function checkUpdates() {
    ...
    try {
      ...
      setMessage(isOnline() ? "The update check could not be completed. Try again." : "Connect to the internet to check for updates.")
    } finally {
    ...
    <div className="rounded-lg border border-cyan-400/20 bg-cyan-500/[0.06] p-5 backdrop-blur-2xl sm:rounded-[36px] sm:p-7">
      ...
      <div className="mt-5">
        <button
          ...
          className="min-h-12 w-full rounded-lg border border-white/10 bg-white/[0.05] px-5 text-sm font-black text-white disabled:opacity-50"
        >
          {status === "checking" ? "Checking..." : "Check for Updates"}
        </button>
      </div>

      {updateAvailable && <div className="mt-5 rounded-lg border border-white/10 bg-black/30 p-4">
