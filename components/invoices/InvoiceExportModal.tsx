...
export function InvoiceExportModal(props: Props) {
  ...
  async function printReport() {
    ...
    try {
      ...
      props.onNotice("System print dialog opened.")
    } catch (nextError) {
  ...
  if (result && dataset) {
    return (
      ...
      {typeof document !== "undefined" && createPortal(
        <ReportPrintSheet dataset={dataset} result={result} />,
        document.body,
      )}
...
function ReportPrintSheet({ dataset, result }: { dataset: InvoiceExportDataset; result: InvoiceReportResult }) {
  return (
    <div className="report-print-sheet" aria-hidden="true">
      <h1>{dataset.businessName}</h1>
      <h2>{result.title}</h2>
      <p>{result.period}</p>
      <div className="report-print-summary">
        <span>Invoices: {dataset.summary.invoiceCount}</span>
        <span>Taxable: {money(dataset.summary.taxableAmount)}</span>
        <span>GST: {money(dataset.summary.totalGst)}</span>
        <span>Total: {money(dataset.summary.grandTotal)}</span>
        <span>Paid: {money(dataset.summary.paidAmount)}</span>
        <span>Outstanding: {money(dataset.summary.outstandingAmount)}</span>
      </div>
      <table>
        <thead><tr><th>Invoice</th><th>Date</th><th>Customer</th><th>Total</th><th>Paid</th><th>Outstanding</th></tr></thead>
        <tbody>
          {dataset.invoiceRows.map((row) => (
            <tr key={row.invoiceId}>
              <td>{row.invoiceNumber}</td><td>{row.invoiceDate}</td><td>{row.customerName}</td>
              <td>{money(row.grandTotal)}</td><td>{money(row.paidAmount)}</td><td>{money(row.dueAmount)}</td>
            </tr>
          ))}
        </tbody>
      </table>
      <footer>Generated by Bezgrow | {result.period}</footer>
    </div>
  )
}
...
function ModalStyles() {
  return (
    <style jsx global>{`
      ...
      .report-print-sheet { display: none; color: #000; background: #fff; }
      @media print {
        @page { margin: 0; }
        html, body { height: auto !important; overflow: visible !important; background: #fff !important; }
        body > * { display: none !important; }
        body > .report-print-sheet { display: block !important; width: 100%; padding: 12mm; box-sizing: border-box; font-family: Arial,sans-serif; }
        .report-print-sheet h1 { margin: 0; font-size: 20px; }
        .report-print-sheet h2 { margin: 5px 0; color: #111; font-size: 16px; }
        .report-print-sheet p { margin: 0 0 8px; font-size: 10px; }
        .report-print-summary { display: grid; grid-template-columns: repeat(3,1fr); gap: 5px; margin: 10px 0; }
        .report-print-summary span { border: 1px solid #aaa; padding: 6px; font-size: 9px; font-weight: 700; }
        .report-print-sheet table { width: 100%; border-collapse: collapse; font-size: 8px; }
        .report-print-sheet thead { display: table-header-group; }
        .report-print-sheet tr { break-inside: avoid; page-break-inside: avoid; }
        .report-print-sheet th { background: #111 !important; color: #fff !important; text-align: left; padding: 5px; }
        .report-print-sheet td { border-bottom: 1px solid #bbb; padding: 5px; }
        .report-print-sheet th:nth-last-child(-n+3), .report-print-sheet td:nth-last-child(-n+3) { text-align: right; }
        .report-print-sheet footer { margin-top: 10px; border-top: 1px solid #aaa; padding-top: 6px; text-align: center; font-size: 8px; }
      }
