...
const seoSections = [
  ...
  [
    ...
    "Generate GST-compliant invoices, tax invoices, retail bills, wholesale invoices, thermal receipts, and A4 invoices with automatic GST calculations.",
  ],
  [
    "Retail POS Software",
    "Fast billing, barcode scanning, customer management, payment tracking, thermal printing, and retail analytics.",
  ],
  [
    "Wholesale Inventory Management",
    "Manage distributors, suppliers, stock transfers, warehouses, purchases, and bulk invoicing.",
  ],
  [
    "ERP Software for Businesses",
    "Combine inventory, billing, customers, analytics, purchases, sales, reporting, and administration in one platform.",
  ],
]
  ...
  return (
    <>
      <header className="relative flex min-h-dvh flex-col overflow-hidden">
        ...
        <section className="relative z-10 mx-auto flex w-full max-w-7xl flex-1 flex-col items-center justify-center px-3 pb-10 pt-6 text-center sm:px-5 sm:pb-12 sm:pt-8 lg:px-8" aria-labelledby="home-heading">
          <div className="landing-fade-up flex max-w-5xl flex-col items-center">
            ...
            <h1 id="home-heading" className="mt-5 max-w-5xl text-center text-4xl font-black leading-[0.98] tracking-tight sm:mt-6 sm:text-6xl lg:text-7xl">
      ...
      <section className="px-5 py-14 lg:px-8" aria-labelledby="seo-overview">
        <div className="mx-auto max-w-6xl">
          ...
          <div className="mt-8 grid gap-4 md:grid-cols-2">
            {seoSections.map(([title, copy]) => {
              const id = title.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/(^-|-$)/g, "")
              return (
                <article key={title} id={id} className="rounded-[24px] border border-white/10 bg-white/[0.035] p-6 shadow-[0_20px_70px_rgba(0,0,0,0.25)]">
            ...
            <article id="why-choose-bezgrow" className="rounded-[24px] border border-cyan-300/20 bg-cyan-300/[0.06] p-6 shadow-[0_20px_70px_rgba(34,211,238,0.08)] md:col-span-2">
              <h2 className="text-2xl font-black">Why Choose Bezgrow</h2>
              <div className="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
                {reasons.map((reason) => (
                  <div key={reason} className="rounded-2xl border border-white/10 bg-black/30 px-4 py-3 text-sm font-bold text-cyan-50">
                    {reason}
                  </div>
                ))}
              </div>
