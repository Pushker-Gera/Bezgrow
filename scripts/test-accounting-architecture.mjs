...
const files = [
  ...
  "lib/accounting/phase3.ts", "lib/offline/local/accounting-phase3.ts",
  "components/accounting/AccountingWorkspace.tsx", "components/accounting/AccountingPhase2Views.tsx", "components/accounting/AccountingPhase3Views.tsx",
]
...
for (const endpoint of ["/api/accounting/assets/create", "/api/accounting/assets/depreciate", "/api/accounting/tds/post", "/api/accounting/tcs/post", "/api/accounting/gst-import", "/api/accounting/e-invoice/prepare", "/api/accounting/bank-statement/import"]) assert.match(localApi, new RegExp(endpoint.replaceAll("/", "\\/")))
for (const group of ["Sales & Receivables", "Purchases & Payables", "Banking", "Tax & GST", "Assets & Analysis", "Reports", "Professional", "Setup"]) assert.match(views, new RegExp(group.replace("&", "&")))
...
assert.match(native, /broken_phase_three_links/)
...
console.log(JSON.stringify({ status: "ok", localAuthority: "SQLite", cloudAccountingImports: 0, accountingNavigation: true, phaseTwoRoutes: true, phaseThreeRoutes: true, serverSafeViewValidation: true, nativeBackupLinks: true }))
