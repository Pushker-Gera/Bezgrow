...
assert.match(gate, /createLocalAppPassword[\s\S]*Choose a device-local App Password/, "A missing credential must allow the owner to choose a local App Password without a licence-import dead end.")
assert.doesNotMatch(gate, /Import \/ Refresh Licence/, "App Lock setup must stay independent from entitlement import and renewal.")
assert.match(gate, /appLockStateFrom\(\{ licenceValid: true, credentialExists: status\.enabled \}\)/, "App Lock state must depend on the secure credential, not subscription validity.")
