...
assert.match(panel, /Update channel[\s\S]*\{buildChannel\}/, "Settings must visibly expose the update channel.")
assert.match(panel, /Installation identity[\s\S]*\{safeBuildId\}/, "Settings must visibly expose a safe non-secret installation identity.")
assert.match(adminLayout, /data-admin-build-identity[\s\S]*adminSafeBuildId/, "Platform Admin must expose the exact safe packaged build identity.")
assert.match(adminLayout, /adminBuildVersion[\s\S]*adminShortCommit[\s\S]*adminBuildPlatform[\s\S]*adminBuildArchitecture[\s\S]*adminBuildChannel/, "Platform Admin diagnostics must include version, commit, platform, architecture, and channel.")
