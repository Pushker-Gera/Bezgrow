...
assert.ok(clientFetch.indexOf("localApiFetch") < clientFetch.lastIndexOf("return fetch("), "Packaged Windows CRUD must attempt the local SQLite API before any browser network fetch.")
assert.match(clientFetch, /if \(await isTauriRuntimeAsync\(\)\)[\s\S]*not implemented in the local SQLite repository/, "A packaged desktop operation must fail closed instead of falling through to cloud ERP storage.")
assert.match(service, /invokeTauri<number>\("desktop_execute"[\s\S]*invokeTauri<T\[]>\("desktop_select"[\s\S]*desktop_execute_transaction/, "Windows CRUD must use the native Tauri SQLite bridge, not localhost HTTP mutations.")
assert.match(localApi, /saveNormalizedProductAtomic[\s\S]*saveNormalizedCustomerAtomic/, "Windows product and customer saves must use row-level repository mutations.")
assert.match(repositories, /saveNormalizedProductAtomic[\s\S]*upsert\(db, "products"[\s\S]*upsert\(db, "inventory_items"/, "Product and opening-stock rows must be committed atomically.")
assert.match(localApi, /archiveNormalizedProductAtomic[\s\S]*updateNormalizedCustomerStatusAtomic/, "Product archive and customer status must remain row-level mutations.")
assert.match(repositories, /collection === "products"[\s\S]*repositories\.products\.merge[\s\S]*collection === "customers"[\s\S]*repositories\.customers\.merge/, "Local product and customer working sets must merge without deleting referenced master rows.")
...
assert.match(installerTest, /Get-InstalledSchemaVersion[\s\S]*Wait-Until[\s\S]*ExpectedSchema[\s\S]*Invoke-AppLaunchCycle 2[\s\S]*ExpectedSchema 22/, "Installer QA must wait for the packaged front end to finish the Accounting Phase 3 migration before closing the upgraded app.")
