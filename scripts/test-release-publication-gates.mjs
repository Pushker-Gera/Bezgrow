...
assert.match(metadataPublisher, /!manualChannel && primaryUpdaterArtifact && !supportsUpdaterMetadata/, "Stable updater publication must fail closed when updater schema columns are unavailable.")
assert.match(writer, /UNSIGNED_MANUAL_RELEASE/, "Published metadata must include the explicit unsigned manual release mode.")
assert.match(workflow, /updater=\$\(\$hasUpdater\.ToString\(\)\.ToLowerInvariant\(\)\)/, "Windows updater signatures must be detected independently of Authenticode.")
