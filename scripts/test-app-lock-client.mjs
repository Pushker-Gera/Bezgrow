...
const ownerChosenStore = storage()
const ownerChosen = client(ownerChosenStore)
await ownerChosen.createLocalAppPassword({ password: "Owner123", deviceId, licenseId, businessId })
assert.equal(await ownerChosen.verifyAppPassword("Owner123"), true, "The owner must be able to provision App Lock locally after onboarding")
assert.equal(ownerChosenStore.secrets.get(secretKey).includes("Owner123"), false, "The chosen App Password must never be stored as plaintext")
