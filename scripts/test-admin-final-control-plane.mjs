...
const checkinConcurrencyMigration = read("supabase/migrations/20260901173000_preserve_license_concurrency_on_checkin.sql")
...
assert.match(atomicLicenseMigration, /admin_mutate_license/, "Atomic licence mutation RPC is missing.")
assert.match(atomicLicenseMigration, /for update/, "Licence mutation RPC must lock the authoritative row.")
assert.match(atomicLicenseMigration, /admin_license_mutations/, "Licence mutation idempotency ledger is missing.")
assert.match(atomicLicenseMigration, /target device is already assigned/, "Device reassignment conflict protection is missing.")
assert.match(atomicLicenseMigration, /2026082201/, "Atomic licence mutation schema version is missing.")
assert.match(atomicReleaseMigration, /2026082203/, "Atomic desktop release schema version is missing.")
...
assert.match(checkinConcurrencyMigration, /tg_table_name\s*=\s*'licenses'/, "Licence timestamp handling must be scoped to the licences table.")
assert.match(checkinConcurrencyMigration, /to_jsonb\(new\)\s*-\s*'updated_at'[\s\S]*to_jsonb\(old\)\s*-\s*'updated_at'/, "Routine check-ins must compare licence state without the timestamp token.")
assert.match(checkinConcurrencyMigration, /new\.updated_at\s*:=\s*old\.updated_at/, "No-op licence updates must preserve the optimistic-concurrency token.")
assert.match(checkinConcurrencyMigration, /else[\s\S]*new\.updated_at\s*:=\s*now\(\)/, "Real control-plane changes must still advance updated_at.")
assert.doesNotMatch(checkinConcurrencyMigration, /(?:delete\s+from|truncate|drop\s+table|public\.(?:customers|products|invoices|orders|organizations)\b)/i, "The check-in concurrency repair must not touch local ERP data or delete control-plane rows.")
