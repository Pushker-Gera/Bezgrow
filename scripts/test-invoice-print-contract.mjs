...
assert.match(engine, /finally \{[\s\S]*actionInFlight\.current = false[\s\S]*setPendingAction\(""\)/, "Print cancellation and failures must always clear the action guard and spinner.")
assert.match(engine, /opened\.status === "cancelled"[\s\S]*print again immediately/, "Print cancellation must be a normal repeatable UI outcome.")
...
assert.match(engine, /WhatsApp opened in manual mode with the professional message/, "WhatsApp must label the supported local-file fallback honestly.")
assert.match(engine, /Bezgrow did not upload it or press Send/, "Manual sharing must state its no-upload and no-send boundary.")
assert.match(engine, /sendCanonicalInvoiceWithWhatsAppBusiness/, "Configured automatic WhatsApp delivery must receive the canonical invoice artifact.")
assert.match(engine, /automaticStatus === "available"/, "The automatic WhatsApp button must be conditional on server-confirmed configuration.")
...
assert.match(rust, /PDFDocument::initWithData[\s\S]*PageScaleDownToFit[\s\S]*setShowsPrintPanel\(true\)[\s\S]*runOperation\(\)/, "macOS must print the validated PDF bytes through a PDFKit system print operation.")
