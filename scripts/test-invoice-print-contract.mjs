...
assert.match(rust, /finish_print_lifecycle[\s\S]*cleanup_started\.swap\(true[\s\S]*operation\.borrow_mut\(\)\.take\(\)[\s\S]*Native print lifecycle completed; terminal=\{terminal\}/, "Deferred cleanup must be idempotent, release native objects, and record the native terminal result.")
